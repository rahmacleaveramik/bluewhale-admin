...
<?php
$qData = "SELECT kd_jenis, judul FROM tb_tugasakhir WHERE (judul LIKE '%$keyword%') ORDER BY judul DESC LIMIT 0, 4";
$hqData = mysql_query($qData) or die (mysql_error());
$rqData = mysql_num_rows($hqData);
if($rqData == '0'){
...
while($rqData = mysql_fetch_array($hqData)){
	...
	$namaBaju[]			= HighLight($rqData[2], $keyword);
	$keteranganBaju[]	= substr($rqData[3], 0, 40)."...";
	$hargaBaju[]		= number_format($rqData[4], 0, ',', '.');
	list($bajuKat) = mysql_fetch_row(mysql_query("SELECT judul FROM tb_jenis WHERE kd_jenis = '$rqData[5]'"));
...
<div class="product_box margin_r40">
	...
	<a style="cursor:pointer;" onclick="javascript: sendRequest('users/detail.php', 'id=<?php echo $idBaju[0]; ?>', 'content', 'div', '');">Detail</a> | <a style="cursor:pointer;" onclick="javascript: sendRequest('users/keranjang.php', 'id=<?php echo $idBaju[0]; ?>', 'content', 'div', '');">Beli</a>
</div>
<?php } ?>
<?php if($idBaju[1] != ""){ ?>
<div class="product_box">	
	...
	<a style="cursor:pointer;" onclick="javascript: sendRequest('users/detail.php', 'id=<?php echo $idBaju[1]; ?>', 'content', 'div', '');">Detail</a> | <a style="cursor:pointer;" onclick="javascript: sendRequest('users/keranjang.php', 'id=<?php echo $idBaju[1]; ?>', 'content', 'div', '');">Beli</a>
</div>
<?php } ?>
<?php if($idBaju[2] != ""){ ?>
...
<div class="product_box margin_r40">
	...
	<a style="cursor:pointer;" onclick="javascript: sendRequest('users/detail.php', 'id=<?php echo $idBaju[2]; ?>', 'content', 'div', '');">Detail</a> | <a style="cursor:pointer;" onclick="javascript: sendRequest('users/keranjang.php', 'id=<?php echo $idBaju[2]; ?>', 'content', 'div', '');">Beli</a>
</div>
<?php } ?>
<?php if($idBaju[3] != ""){ ?>
<div class="product_box">
	...
	<a style="cursor:pointer;" onclick="javascript: sendRequest('users/detail.php', 'id=<?php echo $idBaju[3]; ?>', 'content', 'div', '');">Detail</a> | <a style="cursor:pointer;" onclick="javascript: sendRequest('users/keranjang.php', 'id=<?php echo $idBaju[3]; ?>', 'content', 'div', '');">Beli</a>
</div>
<?php } ?>
<div class="cleaner"></div>
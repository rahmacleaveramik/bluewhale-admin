...
if($_GET['status']=="premium"){
	...
	{
		echo "<script>
		window.location.href='index.php?m=login&redirect=http://localhost/bluewhale-admin/h.php?p=$_GET[p]&staus=$_GET[status]';
		</script>";
	}
	else
	{
		$f = base64_decode($_GET['p']);
		$data = mysql_query ("select * from tb_files where kd_files='$f'");
		...
		if(file_exists($direktori.$filename)){
			...
			if ($file_extension=='php'){
			  echo "<h1>Access forbidden!</h1>
					...
					Silahkan hubungi <a href='mailto:amiki.ac.id'>webmaster</a>.</p>";
			  exit;
			}
			else{
